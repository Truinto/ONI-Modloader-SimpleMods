...
namespace CustomizeBuildings
{
    ...
    internal class AutoMiner_DigBlockingCB
    {
        ...
        {
            try {
                __result = Grid.Foundation[cell] && (!Grid.Transparent[cell] || !CustomizeBuildingsState.StateManager.State.RoboMinerDigThroughGlass) || Grid.Element[cell].hardness >= (byte)150 && !CustomizeBuildingsState.StateManager.State.RoboMinerDigAnyTile;
                return false;
            } catch (Exception) { }
            return true;
        }
    ...
    internal class AutoMiner_ValidDigCell
    {
        ...
        {
            try {
                __result = Grid.Solid[cell] && !Grid.Foundation[cell];
                return false;
            } catch (Exception) { }
            return true;
        }
